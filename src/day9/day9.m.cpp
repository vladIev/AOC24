...
std::vector<BlocksAmount> getInput(std::filesystem::path path)
{
	...
	std::ranges::transform(str, std::back_inserter(output), [](char c) { return c - '0'; });
	return output;
}

std::pair<CheckSum, size_t> updateCheckSum(CheckSum checkSum,
										   size_t checkSumIndex,
										   BlocksAmount fileBlock,
...
auto imitateSwap2(BlocksAmount emptyBlocks,
				  FilesInfo* files) -> std::vector<std::pair<FileId, BlocksAmount>>
{
	std::vector<std::pair<FileId, BlocksAmount>> filledWith;
	while(emptyBlocks > 0)
	{
		auto fileIt = files->rbegin();
		while(fileIt != files->rend() && fileIt->second > emptyBlocks)
			++fileIt;
		if(fileIt == files->rend()) break;
...
auto solve1(const std::vector<BlocksAmount>& input,
			std::map<FileId, BlocksAmount> files) -> uint64_t
{
	...
	for(auto i = 1; !files.empty(); i += 2)
	{
		...
		std::tie(checkSum, checkSumIndex) =
			updateCheckSum(checkSum, checkSumIndex, fileBlocks, forwardFileId);
		...
		{
			std::tie(checkSum, checkSumIndex) =
				updateCheckSum(checkSum, checkSumIndex, fillerBlocks, fillerId);
		}
	}
	return checkSum;
}

auto solve2(const std::vector<BlocksAmount>& input,
			const std::map<FileId, BlocksAmount>& filesOrig) -> uint64_t
{
...
int main()
{
	...
	std::cout << "checksum2 " << solve2(input, files) << std::endl;
